...
```bash
curl "https://brain.sendlime.com/api/v2/messages?limit=10" \
  -H "Authorization: Bearer $SENDLIME_API_KEY"
```
...
const response = await fetch("https://brain.sendlime.com/api/v2/messages", {
  method: "POST",
  headers: {
...
```js
const response = await fetch("https://brain.sendlime.com/api/v2/balance", {
...
BASE_URL = "https://brain.sendlime.com/api/v2"
